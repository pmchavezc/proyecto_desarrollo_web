...
public class ClaseController {
  ...
  @GetMapping("/clases/activas") public List<Clase> activas(){return claseService.clasesActivas();}
  @GetMapping("/mis-clases") public List<Clase> misClases(Authentication auth){
  ...
  @GetMapping("/clases/no-inscritas") public List<Clase> noInscritas(Authentication auth){
    Usuario u = usuarioService.findByEmail(auth.getName()).orElseThrow();
    Estudiante e = usuarioService.findEstudianteByUsuario(u).orElseThrow(); return claseService.clasesNoInscritasActivas(e);
  }
  @PostMapping("/inscripciones/{claseId}") public void inscribirse(Authentication auth, @PathVariable Long claseId){
    Usuario u = usuarioService.findByEmail(auth.getName()).orElseThrow();
